...
export async function renderFiche({ container, params }) {
  ...
  view.innerHTML = `
    <div class="fiche-layout">
      ...
      <aside class="fiche-progress-side">
        ...
        <a class="side-action accent-action" id="printBtn"
  ...
  view.querySelector('#printBtn')?.addEventListener('click', () => {
    window.RM?.toast?.('Téléchargement du PDF…', 'success');
  });
...
// PDF export is now handled by a static file at fiches-pdf/<slug>.pdf,
// generated by scripts/build-pdfs.js (see README). The button is a regular
// download <a> link.
